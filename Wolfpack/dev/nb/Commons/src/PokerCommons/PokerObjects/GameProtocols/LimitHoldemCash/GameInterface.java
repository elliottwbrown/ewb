...
public class GameInterface extends GameXMLInterface {
    ...
    public static String turn(int turn,String PlayerSessionID) throws Exception {
        String msg="";
        if (debugLevel>0) System.out.println("turn:"+PlayerSessionID+":"+turn);
        if (debugLevel>0) System.out.println("gameState before:"+getGameState(PlayerSessionID));
        if (debugLevel>0) System.out.println("handStatus before:"+handStatus);
        if (waitingForTurn) {
        ...
        if (debugLevel>0) System.out.println("roundNo after:"+roundNo);
        if (debugLevel>0) System.out.println("handStatus after:"+handStatus);
        
        if (debugLevel>0) System.out.println("gameState after:"+getGameState(PlayerSessionID));
        return msg;
    }
