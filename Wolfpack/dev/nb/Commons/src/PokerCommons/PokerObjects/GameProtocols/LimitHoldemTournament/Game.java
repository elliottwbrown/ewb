...
public class Game extends GameImplementation implements GameValues {
    ...
    public static float smallBlindAmt=1,bigBlindAmt=2;
    public static float[] raiseAmt={0,20,20,40,40};
    public static final int turnLength=60000,maxRaisesPerRound=4,debugLevel=1;
    public static boolean keepPlayingHands=true,waitingForTurn=false;
    private static String
            balancesLoggerFileName="c:\\temp\\Balances.log";//,
    ...
    public static void setBlinds() throws Exception {
        if (gameID<11) {
            smallBlindAmt=10;
            bigBlindAmt=15;
            raiseAmt[1]=raiseAmt[2]=15;
            raiseAmt[3]=raiseAmt[4]=30;
        } else if (gameID<21) {
            smallBlindAmt=15;
            bigBlindAmt=30;
            raiseAmt[1]=raiseAmt[2]=30;
            raiseAmt[3]=raiseAmt[4]=60;
        } else if (gameID<31) {
            smallBlindAmt=25;
            bigBlindAmt=50;
            raiseAmt[1]=raiseAmt[2]=50;
            raiseAmt[3]=raiseAmt[4]=100;
        } else if (gameID<41) {
            smallBlindAmt=50;
            bigBlindAmt=100;
            raiseAmt[1]=raiseAmt[2]=100;
            raiseAmt[3]=raiseAmt[4]=200;
        } else if (gameID<51) {
            smallBlindAmt=100;
            bigBlindAmt=200;
            raiseAmt[1]=raiseAmt[2]=200;
            raiseAmt[3]=raiseAmt[4]=400;
        } else {
            smallBlindAmt=150;
            bigBlindAmt=300;
            raiseAmt[1]=raiseAmt[2]=300;
            raiseAmt[3]=raiseAmt[4]=600;
        }
    }
    ...
    public static void startHand() throws Exception {
        ...
        // choose small blind
        if (players.size()==2) smallBlind=dealer;
        else smallBlind=getPlayerToTheLeft(dealer);
    ...
    public static void startRound() throws Exception {
        ...
        if (roundNo<END_OF_HAND ) {
            if (roundNo>DEAL) {
                for (Player p : players.values()) p.setCallAmt(0);
                currentPlayer=getPlayerInHandToTheLeft(dealer);
            }
            if (roundNo==DEAL) dealHoleCards();
            if (roundNo==FLOP) dealFlop();
            if (roundNo==TURN) dealTurn();
            if (roundNo==RIVER) dealRiver();
        }
    ...
    public static void endRound() throws Exception {
        ...
        roundNo++;
        if (roundNo>=END_OF_HAND || countPlayersInGame()==1) endHand();
        else startRound();
    }
    ...
    public static void endHand() throws Exception {
        ...
        if (countPlayersInHand()==1) {
            ...
            c=0;
            for (Player p : players.values()) if (p.isInCurrentHand()) hands[c++]=p.getHoleCards();
            try {
            ...
            for (Player p : players.values()) {
                if (p.isInCurrentHand()) {
                    ...
                    if (winners[c++]) numWinners++;
                }
            }
            if (numWinners==1) handHistory+="  There is one winner."+"\n";
            else handHistory+="  There are "+numWinners+" winners."+"\n";
            c=0;
            for (Player p : players.values()) {
        ...
        for (Player p : players.values()) finishingBalances+=gameID+","+p.getName()+","+p.getBalance()+"\n";
    ...
    public static void processTurn(int turn) throws Exception {
        ...
        if (turn==CHECKFOLD) processCheckFold();
        else if (turn==CALL) processCall();
        else if (turn==RAISE) {
            numRaisesThisRound++;
            //if ((currentPlayer.getNumberRaises()+1)<=maxRaisesPerRound) processRaise();
            if (numRaisesThisRound<=maxRaisesPerRound) processRaise();
            else processCall();
        }
        if (countPlayersInGame()==1) endRound();
        else
            if (isRemainingCalls()) currentPlayer=getPlayerInHandToTheLeft(currentPlayer);
            else endRound();
    }
    ...
    private static void processRaise() throws Exception {
        if (currentPlayer.getBalance()>0) {
            ...
            if (currentPlayer.getCallAmt()==0f) handHistory+="  "+currentPlayer.getName()+" bet "+raiseAmt[roundNo]+"\n";
            else handHistory+="  "+currentPlayer.getName()+" raised "+raiseAmt[roundNo]+"\n";
    ...
    private static void processCall() {
        if (currentPlayer.getCallAmt()==0f) {
            ...
            if (currentPlayer.getBalance()>currentPlayer.getCallAmt()) {
                ...
                // all in
                float balance= currentPlayer.getBalance();
                ...
                pot+=balance;
                float difference=currentPlayer.getCallAmt()-balance;
                pot-=balance;
