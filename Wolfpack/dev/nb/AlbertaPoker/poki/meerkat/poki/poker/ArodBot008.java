...
//	}
	...
	public synchronized void newGame(
		...
		if (gi2.getNumPlayers() == 0) {
			for (int i = 0; i < gi.getNumPlayers(); i++) {
				gi2.addPlayer(gi.getPlayerInfo(i).getName(),gi.getPlayerInfo(i).getName());
			}
		}
	...
	public synchronized void actionEvent(int pos, int action, int amount) {
		flag = true;
		debug("*****actionEventCalled");
		// if we are still active
		if (gi.activePlayer(ID)) {
		  debug("*************ID " + gi.activePlayer(ID));
			...
			if ((gi.getStage() == Holdem.PREFLOP) && (orbit == 0)) {
				...
				if ((action == Holdem.U_CHECK)
					|| (action == Holdem.U_CALL)
					|| (action == Holdem.U_RAISE)) {
			...
			switch (action) {
				case Holdem.U_BBLIND :
					{
						gi2.bigBlind();
						break;
					}
				case Holdem.U_SBLIND :
					{
						gi2.smallBlind();
						break;
					}
			...
			if ((pos == BB) && (action != Holdem.U_BBLIND)) {
				orbit++;
			}
			gi2.advanceCurrentPlayer();
		}
   ...
   //debug("***actionRecord " + gr.getAction(gr.getPlayerIndex(""),));
...
//		}
	...
	public synchronized ProbTriple preFlopAction(
		...
		int preflopGroupLocal) {
			...
			debug("preFlopGroup=" + preflopGroupLocal);
