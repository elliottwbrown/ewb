...
public class MainApp extends MainForm implements GameValues {
    ...
    private void resetCards() {
        ...
        holeCard2.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
        communityCard1.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
        communityCard2.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
        communityCard3.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
        communityCard4.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
        communityCard5.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
        oppCard1.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
        oppCard2.setIcon(new ImageIcon(cardsLocation+"FD.gif"));
    }
    ...
    public void processGUIEvent(java.awt.event.ActionEvent evt) {
        ...
        } else if ("Post Blind".equals(eventName)) {
            StartButtonPressed();
        } else if (eventName==CallButton.getText()) {
            CallButtonPressed();
        } else if (eventName.equals(CheckButton.getText())) {
            CheckFoldButtonPressed();
        } else if (eventName.equals(RaiseButton.getText())) {
            RaiseButtonPressed();
        } else if (eventName.equals(ResetButton.getText())) {
            StatusBar.setText("resetting");
            ResetButtonPressed();
        } else if ("getGameState".equals(eventName)) {
    ...
    public void joinGame()  {
        try {
            if (debug>0) System.out.println("client "+client);
            String response=client.sendOperation("join","name="+nameTextField.getText());
            if (debug>0) System.out.println("response "+response);
            String msg= XMLUtilities.getMessageBody(response);
            if (debug>0) System.out.println("msg "+msg);
    ...
    synchronized public void getGameState()  {
        ...
        while (!readSuccesfully) {
            try {
                if (debug>0) System.out.println(">>> gettingGameState");
                lastResponse=response;
                response=client.sendOperation("getGameState");
                if (debug>0) System.out.println(">>> done. Parsing GameState");
        ...
        if (debug>0) System.out.println(">>> gettingGameState finished");
    }
    ...
    public void parseGameState() {
        if (lastResponse!=response) {
            try {
                ...
                balance=Float.parseFloat(XMLUtilities.getTag(response,"<sessionID>"+client.getSessionID()+"</sessionID><name>"+myName+"</name><balance>"));
                if (handStatus>HAND_NOT_STARTED) {
                    ...
                    if (handStatus==HAND_STARTED) {
                        ...
                        currentPlayer=XMLUtilities.getTag(
                                XMLUtilities.getTag(response,"<turnInfo>")
                                ,"<player>");
                        ...
                        if (response.contains("<cardInfo>")) {
                            ...
                            if (roundNo>=DEAL) {
                                ...
                                holeCards.add(new Card(XMLUtilities.getTag(response,"<card2>")));
                            }
                            if (roundNo>=FLOP) {
                                communityCards.add(new Card(XMLUtilities.getTag(response,"<card3>")));
                                communityCards.add(new Card(XMLUtilities.getTag(response,"<card4>")));
                                communityCards.add(new Card(XMLUtilities.getTag(response,"<card5>")));
                            }
                            if (roundNo>=TURN) communityCards.add(new Card(XMLUtilities.getTag(response,"<card6>")));
                            if (roundNo>=RIVER) {
                                communityCards.add(new Card(XMLUtilities.getTag(response,"<card7>")));
                                try {
                                    opponentsCards.add(new Card(XMLUtilities.getTag(response, "<card8>")));
                                    opponentsCards.add(new Card(XMLUtilities.getTag(response, "<card9>")));
                                } catch (Exception e) {
                                    System.out.println("No card 8");
                                }
                            }
                            callAmount = 0;
                ...
                if (handStatus>HAND_NOT_STARTED) {
                    ...
                    if (handStatus==HAND_STARTED) {
                        ...
                        if (response.contains("<cardInfo>")) {
                            ...
                            holeCard2.setIcon(new ImageIcon(cardsLocation+holeCards.get(1).getCardShortestName()+".gif"));
                            if (roundNo>=FLOP) {
                                communityCard1.setIcon(new ImageIcon(cardsLocation+communityCards.get(0).getCardShortestName()+".gif"));
                                communityCard2.setIcon(new ImageIcon(cardsLocation+communityCards.get(1).getCardShortestName()+".gif"));
                                communityCard3.setIcon(new ImageIcon(cardsLocation+communityCards.get(2).getCardShortestName()+".gif"));
                            }
                            if (roundNo>=TURN) communityCard4.setIcon(new ImageIcon(cardsLocation+communityCards.get(3).getCardShortestName()+".gif"));
                            if (roundNo>=RIVER) communityCard5.setIcon(new ImageIcon(cardsLocation+communityCards.get(4).getCardShortestName()+".gif"));
                        }
                    }
                    if (handStatus==HAND_FINISHED) {
                        try {
                            oppCard1.setIcon(new ImageIcon(cardsLocation + opponentsCards.get(0).getCardShortestName() + ".gif"));
                            oppCard2.setIcon(new ImageIcon(cardsLocation + opponentsCards.get(1).getCardShortestName() + ".gif"));
                        } catch (Exception e) {
                            System.out.println("no opp cards");
                            //e.printStackTrace();
                        }
                        updateThread.setKeepUpdating(false);
                        StartButton.setEnabled(true);
